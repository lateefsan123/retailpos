...
const SelectUser: React.FC = () => {
  ...
  const fetchUsers = async () => {
    ...
    try {
      ...
      const usersWithBranches = await Promise.all((data || []).map(async (userRecord) => {
        if (userRecord.branch_id) {
          ...
          const displayName =
            branchDisplayMap[userRecord.branch_id] ??
            buildBranchDisplayName(branchData)
  ...
  const handlePinSubmit = async (e: React.FormEvent) => {
    ...
    if (isValid) {
      ...
      setPinError('Incorrect PIN. Please check your PIN and try again.');
    }
  };
                    ...
                    <form onSubmit={handlePinSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 16, maxWidth: '300px', margin: '0 auto' }}>
                      <div>
                        ...
                        {capsLockOn && (
                          ...
                          >
                            <i className="fa-solid fa-exclamation-triangle" style={{ color: '#f59e0b', fontSize: 14 }} />
                            <p style={{ fontSize: 14, color: '#f59e0b', margin: 0, fontWeight: 500 }}>Caps Lock is ON</p>
                          </div>
                        )}
                      </div>
                ...
                <div style={{ 
                  ...
                  <form onSubmit={handlePasswordSubmit} style={{ width: '100%', maxWidth: '400px' }}>
                    ...
                    {capsLockOn && (
                      ...
                      >
                        <i className="fa-solid fa-exclamation-triangle" style={{ color: '#fbbf24', fontSize: '14px', textShadow: '0 1px 2px rgba(0,0,0,0.8)' }} />
                        <p style={{ fontSize: '14px', color: '#fbbf24', margin: 0, fontWeight: 500, textShadow: '0 1px 2px rgba(0,0,0,0.8)' }}>Caps Lock is ON</p>
                      </div>
                    )}
                    ...
                    {branches[currentBranchIndex] && (
                      ...
                      >
                        ...
                        <div style={{
                          ...
                          {getBranchLabel(branches[currentBranchIndex])}
                        </div>
                      </div>
                    )}
                        ...
                        <div style={{ display: 'flex', flexDirection: 'column', gap: 16, height: '100%' }}>
                          ...
                          <div style={{ display: 'flex', flexDirection: 'column', gap: 8, flex: 1 }}>
                            ...
                            <div style={{ 
                              ...
                              <span>{u.branch_display_name || u.branch_name || 'No Branch Assigned'}</span>
                            </div>
