...
interface Product {
  ...
  pending_confirmation?: boolean // true if product needs confirmation from Product Database
}
...
const Products = () => {
  ...
  // Fetch pending products (products that need confirmation from Product Database)
  ...
  // Handle confirming a pending product
  const handleConfirmPendingProduct = async (product: Product, updatedData: Partial<Product>) => {
    if (businessId == null) {
      setError('Please select a business before confirming products')
      return
    }

    try {
      console.log('Confirming product:', product.product_id)
      ...
      const updatePayload = {
        ...
        last_updated: new Date().toISOString(),
        // Set branch_id to the currently selected branch, or keep as null for all branches
      ...
      if (error) {
        ...
        alert('Failed to confirm product: ' + error.message)
        throw error
      }

      console.log('Product updated successfully:', data)

      // Verify the product is now confirmed
      ...
      setPendingProducts(pendingProducts.filter(p => p.product_id !== product.product_id))
      setConfirmingProduct(null)
      
      // Refresh all data
      await fetchProducts(currentPage, itemsPerPage, searchTerm, selectedCategory, activeSummaryFilter)
      await fetchPendingProducts()
      await fetchAllProductsForSuggestions(true)
      await fetchSummaryStats(true)
      
      alert('Product confirmed and added to inventory!')
    } catch (err) {
      console.error('Failed to confirm product:', err)
      setError(err instanceof Error ? err.message : 'Failed to confirm product')
    }
  }

  // Handle rejecting a pending product
  const handleRejectPendingProduct = async (productId: string) => {
    if (businessId == null) {
      setError('Please select a business before rejecting products')
      return
    }
    ...
    try {
      ...
      alert('Pending product rejected and removed.')
    } catch (err) {
      ...
      {pendingProducts.length > 0 && (
        <div style={{
          ...
          <div style={{
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
              <i className="fa-solid fa-clock" style={{ fontSize: '20px', color: '#f59e0b' }}></i>
              <h2 style={{
                fontSize: '18px',
                fontWeight: 'bold',
                color: '#92400e',
            ...
            >
              <i className={`fa-solid fa-chevron-${showPendingSection ? 'up' : 'down'}`} style={{ fontSize: '14px' }}></i>
            </button>
          </div>
                    ...
                    <div style={{ display: 'flex', gap: '12px', marginBottom: '12px' }}>
                      {product.image_url ? (
                        ...
                        <div style={{
                          ...
                          <i className="fa-solid fa-image" style={{ fontSize: '24px', color: '#9ca3af' }}></i>
                        </div>
                      )}
                      <div style={{ flex: 1, minWidth: 0 }}>
                        ...
                        {product.barcode && (
                          <p style={{
                            ...
                            <i className="fa-solid fa-barcode" style={{ marginRight: '4px' }}></i>
                            {product.barcode}
                          </p>
                        )}
                      </div>
                      ...
                      <button
                        onClick={() => {
                          if (confirm('Are you sure you want to reject this product?')) {
                            handleRejectPendingProduct(product.product_id)
                          }
                        }}
                        style={{
                          background: '#ef4444',
            ...
            <form onSubmit={(e) => {
              ...
              <div style={{
                ...
                {confirmingProduct.image_url ? (
                  <img
                    src={confirmingProduct.image_url}
                  ...
                  <div style={{
                    ...
                    <i className="fa-solid fa-image" style={{ fontSize: '32px', color: '#9ca3af' }}></i>
                  </div>
                )}
                <div style={{ flex: 1 }}>
                  <h3 style={{
